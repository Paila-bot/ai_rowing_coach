...
class BackgroundSubtractor:
    def __init__(self, shape=None, alpha=0.01, history_size=50):
        """
        Initialize background subtractor with rowing-specific pose estimation
    ...
    def _ensure_shape_compatibility(self, frame):
        """Ensure frame and background have compatible shapes"""
        frame = frame.astype(np.float32)

        if self.bg is None:
            # Initialize background with first frame's shape
    ...
    def update(self, frame):
        ...
        # Store frame in history for median background calculation
        ...
        if not self.initialized:
            ...
            if self.frame_count == 0:
                ...
                # Use running average for initial frames
                self.bg = (self.bg * self.frame_count + frame) / (self.frame_count + 1)

            self.frame_count += 1

            # After collecting enough frames, switch to median background
            if self.frame_count >= 10:
                self.initialized = True
                if len(self.frame_history) >= 5:
                    # Use median of recent frames as initial background
                    self.bg = np.median(np.array(list(self.frame_history)), axis=0)
        else:
    ...
    def subtract(self, frame, threshold=30, use_adaptive=True):
        """
        Subtract background and create foreground mask
        ...
        if use_adaptive and self.initialized:
            ...
            # Calculate local mean and std of differences
            kernel_size = 15
            kernel = np.ones((kernel_size, kernel_size), np.float32) / (kernel_size * kernel_size)
            local_mean = cv2.filter2D(diff, -1, kernel)
            local_std = np.sqrt(cv2.filter2D((diff - local_mean) ** 2, -1, kernel))

            # Adaptive threshold = base_threshold + k * local_std
            adaptive_threshold = threshold + 1.5 * local_std
            fg_mask = diff > adaptive_threshold
        else:
            # Simple thresholding
    ...
    def get_cleaned_mask(self, frame, threshold=30, min_area=100):
        """
        Get cleaned foreground mask with morphological operations
    ...
    def extract_rowing_joints(self, points: List[Tuple[int, int]],
        ...
        Args:
            points: List of (y, x) coordinates from connected component
            frame_shape: Shape of the frame (height, width)
        Returns:
            Dictionary of joint positions with rowing-specific joints
        """
        if not points:
